...
export async function cleanDb(req: Request, res: Response) {
  ...
  try {
    // Delete resources
    await pool.query(
      'DELETE FROM resources WHERE uploaded_by IN (SELECT id FROM users WHERE LOWER(email) = $1)',
      [email.toLowerCase()]
    );
    // Delete sessions
    await pool.query(
      'DELETE FROM sessions WHERE created_by IN (SELECT id FROM users WHERE LOWER(email) = $1)',
      [email.toLowerCase()]
    );
    // Delete users
    const result = await pool.query(
      'DELETE FROM users WHERE LOWER(email) = $1 RETURNING id, email, name',
      [email.toLowerCase()]
    );
