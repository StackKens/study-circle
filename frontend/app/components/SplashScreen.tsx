...
export default function SplashScreen({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(135deg, #0a0f1e 0%, #0d1428 60%, #111a30 100%)",
        opacity: phase === "exit" ? 0 : 1,
        transition:
          phase === "exit" ? "opacity 0.5s ease" : "opacity 0.4s ease",
      }}
    >
      {/* Ambient glow rings */}
      ...
      >
        ...
        <div className="relative">
          <div className="w-20 h-20 rounded-2xl bg-teal-500/20 border border-teal-400/30 flex items-center justify-center backdrop-blur-sm shadow-2xl">
            <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
              <circle cx="14" cy="14" r="5" fill="#2dd4bf" opacity="0.65" />
              <circle cx="26" cy="14" r="5" fill="#5eead4" opacity="0.65" />
              <circle cx="20" cy="26" r="5" fill="#14b8a6" opacity="0.65" />
              <line
                x1="14"
                y1="14"
                x2="26"
          ...
          {/* Sparkle dots */}
          {[
            { top: -8, right: -8, delay: "0.8s", size: 4, color: "#2dd4bf" },
            { bottom: -6, left: -10, delay: "1.1s", size: 3, color: "#5eead4" },
            { top: 4, left: -14, delay: "1.4s", size: 3, color: "#14b8a6" },
          ].map((s, i) => (
            <div
              key={i}
        ...
        <div className="text-center">
          ...
          >
            Study<span style={{ color: "#2dd4bf" }}>Circle</span>
          </h1>
          <p
