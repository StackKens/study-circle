...
import {
  Megaphone, ArrowRight, Users, BookOpen,
  ChevronRight, X, Bell,
} from "lucide-react";
...
export default function InstructorAnnouncementsHub() {
  ...
  const totalAnnouncements = courses.reduce((s, c) => s + (c.announcement_count ?? 0), 0);
  const totalStudents      = courses.reduce((s, c) => s + (c.student_count      ?? 0), 0);

  function startPost() {
    setStep("course"); setSelectedCourse(null);
    setTitle(""); setContent(""); setError(""); setSuccess("");
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${API_URL}/courses/${selectedCourse.id}/announcements`, {
        method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          { icon: Megaphone, value: totalAnnouncements, label: "Total Posted",    color: "bg-teal-50 text-teal-600"   },
      ...
      <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 mb-6 flex items-start gap-3">
        ...
        <p className="text-sm text-amber-700 leading-relaxed">
          <span className="font-semibold">Announcements go only to enrolled students.</span> Students who haven't enrolled in the course will not see them.
          Each announcement is tied to one specific course.
        </p>
      </div>
      ...
      {step === "course" && (
        <div className="bg-white rounded-2xl border border-slate-200 p-5 mb-6 shadow-sm">
          <div className="flex items-center justify-between mb-1">
            <p className="font-semibold text-slate-900">Step 1 — Pick a course</p>
            <button onClick={() => setStep(null)} className="p-1.5 hover:bg-slate-100 rounded-lg cursor-pointer">
              <X size={16} className="text-slate-400" />
            </button>
          </div>
          <p className="text-xs text-slate-400 mb-4">Which course is this announcement for? Only enrolled students will see it.</p>
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    {c.code || "No code"} · {c.student_count} student{c.student_count !== 1 ? "s" : ""} · {c.announcement_count} announcement{c.announcement_count !== 1 ? "s" : ""}
                  </p>
                </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 mb-6 shadow-sm">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-1.5 text-sm">
              ...
              <span className="font-semibold text-slate-900">Step 2 — Write announcement</span>
            </div>
            <button onClick={() => setStep(null)} className="p-1.5 hover:bg-slate-100 rounded-lg cursor-pointer">
              <X size={16} className="text-slate-400" />
            </button>
          ...
          <div className="mt-2 mb-4">
            <span className="text-xs font-medium px-2 py-0.5 rounded-md bg-teal-50 text-teal-700">{selectedCourse.title}</span>
            <span className="text-xs text-slate-400 ml-2">· {selectedCourse.student_count} enrolled student{selectedCourse.student_count !== 1 ? "s" : ""} will be notified</span>
          </div>

          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Title</label>
              <input value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="e.g. Assignment 2 deadline extended" className={inputClass} />
            </div>
            <div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-sm text-slate-400 mt-1 mb-4">Create a course to start posting announcements to your students.</p>
