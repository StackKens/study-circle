...
export default function InstructorDiscussionsHub() {
  const { token } = useAuth();
  ...
  const totalDiscussions = courses.reduce((s, c) => s + (c.discussion_count ?? 0), 0);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Discussions</h1>
        <p className="text-sm text-slate-500 mt-1">
          Answer student questions and keep conversations going across your courses
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-col">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Total Threads</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-col">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            <HelpCircle size={15} /> Answer Questions
          </Link>
          <Link
            to="/dashboard/instructor/courses"
          ...
          >
            <CheckCircle2 size={15} className="text-teal-600" /> Mark as Answered
          </Link>
          <Link
            to="/dashboard/instructor/courses"
            className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors"
          >
            <Lightbulb size={15} className="text-amber-500" /> Start a Thread
          </Link>
        </div>
        <p className="text-xs text-slate-400 mt-2">
          Pick a course below to view and reply to its discussions.
        </p>
      </div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-6 flex items-start gap-3">
        ...
        <p className="text-sm text-slate-600 leading-relaxed">
          <span className="font-semibold text-slate-800">Tip:</span> Responding quickly to student
          questions increases engagement. Try to answer open threads within 24 hours.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-sm text-slate-400 mt-1 mb-4">
            Once you create a course, students can start posting questions here.
          </p>
          <Link
            to="/dashboard/instructor/courses"
        ...
        <div className="space-y-3">
          ...
          {courses.map((c) => (
            <Link
              key={c.id}
              to={`/dashboard/instructor/courses/${c.id}/discussions`}
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    {c.code || "No code"} · {c.student_count} student{c.student_count !== 1 ? "s" : ""} · {c.discussion_count} thread{c.discussion_count !== 1 ? "s" : ""}
                  </p>
                </div>
              </div>
